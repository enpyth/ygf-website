...
export default function GoogleMap({ stores, height = '500px', initialCenter, initialZoom }: GoogleMapProps) {
    ...
    const initialStoresRef = useRef<Store[] | null>(null)
    ...
    const initMap = async () => {
        ...
        try {
            ...
            // 创建标记和信息窗口（仅使用首次渲染时的 stores）
            markerMapRef.current = {}
            const storesToUse = (initialStoresRef.current ?? stores)
    ...
    useEffect(() => {
        ...
        initialStoresRef.current = stores
        initMap()
